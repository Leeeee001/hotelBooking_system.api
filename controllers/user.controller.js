...
const bookRoom = async (req, res) => {
    try {
        ...
        if (!roomId || !checkInDate || !checkOutDate) {
            return res.status(400).json({ error: "roomId, checkInDate, and checkOutDate are required" });
        }
        ...
        if (availabilities.length !== dateList.length) {
            return res.status(400).json({ error: "Room not available for entire duration" });
        }
...
module.exports = { getProfile, updateProfile, getAvailableRooms, getRoomDetails, bookRoom, getMyBookings, cancelBooking, deleteAccount }
